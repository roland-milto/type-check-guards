...
Valider brukerleverte eller dynamisk bygde samlinger (f.eks. flere fil-lesestrømmer) før du piper, gjenopptar eller på
annen måte opererer på dem som en gruppe.
...
- Gir en enkel `true`/`false`-sjekk for å validere stream-samlinger før behandling.
...
if (areStreams(input)) {
  // input ke lenaneo le tletšego la dilo tša Stream
...
<small>Filen ble generert 30 January 2026 at 23:35:01 (UTC) ved bruk av *
