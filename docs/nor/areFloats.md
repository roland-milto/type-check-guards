...
`areFloats` sjekker om en gitt matrise er utfylt og om alle elementene er flyttall.

### Brukstilfelle

Bruk `areFloats` når du mottar en `unknown[]` (f.eks. fra JSON, spørringsparametere eller eksterne API-er) og du må
sikre at det er en utfylt matrise der hvert element er et flyttall før du kjører numerisk logikk som
...
> returnerer `false` for tomme matriser og for ethvert element som ikke er et flyttall.

### Fordeler

- Returnerer `true` bare når inndata er en ikke-tom matrise og hvert element er et flyttall.
- Feiler raskt: returnerer `false` så snart et element som ikke er et flyttall blir funnet.
...
<small>Filen ble generert 30 January 2026 at 15:58:57 (UTC) ved bruk av *
