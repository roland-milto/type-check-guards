...
Valider kjøretidsdata (f.eks. parsede JSON-data, plugin-inndata eller løst typet konfigurasjon) for å sikre at det er en
...
>
> Bruk `areWeakMaps` til å validere ukjent inndata før du behandler det som en ikke-tom `WeakMap[]`; den returnerer
`false` for tomme matriser.

### Fordeler

- Sikrer at hvert element i den angitte matrisen er en `WeakMap`-instans.
- Returnerer `false` for tomme matriser, noe som hindrer utilsiktet aksept av «ingen data» som gyldig inndata.
...
if (areWeakMaps(list)) {
  // list ke lenaneo le le se nago selo la disampole tša WeakMap
  for (const wm of list) {
    wm.set({}, 1);
  }
} else {
  // ga se WeakMap[] ye e se nago selo
}
...
<small>Filen ble generert 30 January 2026 at 13:38:44 (UTC) ved bruk av *
