...
- Avviser tomme arrayer med hensikt, og forhindrer utilsiktet godkjenning av «ingen data» som gyldig input.
...
if (areMaps(items)) {
  // items er garantert å vera ein ikkje-tom tabell av Map-instansar ved køyretid
  for (const m of items) {
    m.set("x", 42);
  }
} else {
  // false for: tomme tabellar, eller tabellar som inneheld ein ikkje-Map-verdi
}
...
<small>Filen ble generert 31 January 2026 at 16:13:55 (UTC) ved bruk av *
