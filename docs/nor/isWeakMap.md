...
Bruk `isWeakMap` når du tar imot en `unknown`-verdi (f.eks. fra et offentlig API, et pluginsystem eller dynamisk
...
- Bidrar til å forhindre feilbruk av API-er som krever en `WeakMap` ved å returnere `true`/`false` i stedet for å kaste
  en feil.
...
console.log(isWeakMap(a)); // sann
console.log(isWeakMap(b)); // usann
```
...
<small>Filen ble generert 30 January 2026 at 13:26:14 (UTC) ved bruk av *
