...
Bruk `isWeakSet` når du tar imot utypet input (f.eks. fra eksterne API-er, dynamisk konfigurasjon eller `unknown`
-verdier) og du må verifisere at det er en `WeakSet` før du bruker `WeakSet`-spesifikke operasjoner.

> **Merknad for TypeScript-brukere:**
>
> Bruk `isWeakSet` for å snevre inn en `unknown`-verdi til `WeakSet<object>` ved kjøretid; merk at `WeakSet` bare kan
...
- Fungerer med enhver `unknown`-input og returnerer et tydelig boolsk resultat (`true`/`false`).
...
if (isWeakSet(a)) {
  // a ke WeakSet ka nako ya go šoma
}
```
