...
پیش از ادامه، بررسی کنید که یک فهرست از feature flagها، چک‌ها یا نتایج guard همگی `false` باشند (مثلاً تأیید کنید هیچ
شرطِ مسدودکننده‌ای وجود ندارد).
...
- برای غیرآرایه‌ها یا آرایه‌های خالی `false` برمی‌گرداند، چون از طریق `isFilledArray` وجود یک آرایه پُر را الزام می‌کند.
...
<small>این فایل در 31 January 2026 at 16:18:20 (UTC) با استفاده از *
