...
رسیدگی به مقادیری که از پلاگین‌ها، importهای پویا، یا APIهای با نوع‌دهی سست برگردانده می‌شوند.
...
- به محافظت از مسیرهای کد که به یک نمونهٔ واقعی `Promise` نیاز دارند کمک می‌کند و به‌صورت قابل پیش‌بینی `true` یا
  `false` برمی‌گرداند.
- با الزام به یک نمونهٔ واقعی `Promise` از مثبت‌های کاذبِ ناشی از اشیای «thenable» (مثلاً `{ then() {} }`) جلوگیری
  می‌کند.
...
<small>این فایل در 30 January 2026 at 23:53:59 (UTC) با استفاده از *
