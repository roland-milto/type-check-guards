...
`isNumeric` با بررسی نوعِ حل‌شدهٔ `value` در برابر `NUMERIC_TYPES` تعیین می‌کند که آیا `value` داده‌شده عددی در نظر
گرفته می‌شود یا نه.

### مورد استفاده

از `isNumeric` برای اعتبارسنجی ورودی‌ها (مثلاً payloadهای API، مقادیر فرم، پیکربندی) پیش از انجام عملیات عددی استفاده
کنید، و برای پذیرش سازگارِ نوع‌های شبیه‌عدد (مانند `BigInt`) مطابق با `NUMERIC_TYPES`.
...
- از چندین نمایش عددی (مثلاً `number`، `BigInt`) مطابق با تعریف `NUMERIC_TYPES` پشتیبانی می‌کند.
...
- `value`: مقداری که باید برای نوع عددی بودن بررسی شود.
...
for (const v of inputs) {
  if (isNumeric(v)) {
    //‎‎ v بر اساس قوانین نوع‌دهی کتابخانه عددی در نظر گرفته می‌شود
...
<small>این فایل در 6 February 2026 at 15:54:09 (UTC) با استفاده از *
