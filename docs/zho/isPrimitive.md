...
在运行时验证输入（例如 API 负载字段、配置值或用户提供的数据），确保在序列化、记录日志或应用仅适用于原始类型的操作之前，该值为原始类型。

> **给 TypeScript 用户的提示：**
>
> 在将 `unknown` 输入当作对象或函数处理之前，使用 `isPrimitive` 进行守卫；它对原始类型返回 `true`，对对象和函数返回
`false`。
...
- 在执行仅适用于对象的操作之前，帮助缩小 `unknown` 值的范围。
...
<small>该文件于 2026年1月30日 23:58:32 (UTC) 使用 *
