...
`arePrimitives` 用于评估所提供的非空数组中的所有元素是否都是原始类型。

### 使用场景

在序列化、哈希、记录日志，或将数据传递给不得接收对象的 API 之前，验证传入数据（例如查询参数、CSV 行值，或 ID/标签列表）仅包含原始值。

> **给 TypeScript 用户的提示：**
>
> 当你需要在进一步处理之前确保 `unknown[]` 仅包含原始值（string、number、bigint、boolean、symbol、undefined 或 null）时，请使用
`arePrimitives`。
...
<small>该文件于 2026年1月31日 00:07:24 (UTC) 使用 *
