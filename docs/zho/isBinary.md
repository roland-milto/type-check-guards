...
在进一步处理之前，验证用户提供的字符串（例如表单字段、CLI 参数、配置值），确保它们仅表示二进制数字，并且可选带 `0b`/`0B` 前缀。

> **给 TypeScript 用户的提示：**
>
> 在解析或将字符串转换为 `BigInt`/`Number` 之前，将 `isBinary` 用作类型守卫以避免无效输入。
...
<small>该文件于 2026年1月31日 23:12:01 (UTC) 使用 *
