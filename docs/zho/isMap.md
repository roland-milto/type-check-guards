...
当你收到一个 `unknown` 值（例如来自 JSON 解析、外部 API 或用户输入）并且需要在执行 `Map` 操作之前确保它是 `Map` 时，使用
`isMap`。
...
- 通过保护需要使用 `Map` 方法（如 `get`、`set`、`has`）的代码路径，帮助防止类型错误。
- 在处理 `unknown` 输入时，它非常适合作为轻量级的验证步骤。
...
<small>该文件于 2026年1月31日 16:42:46 (UTC) 使用 *
