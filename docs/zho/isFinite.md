...
确定给定的 `value` 是否为有限的 `number`。

### 使用场景

在进行数值计算之前，使用 `isFinite` 验证未知输入（例如来自 JSON、表单或 API 的输入），确保该值是一个真实且有限的数字。
...
<small>该文件于 2026年1月30日 16:32:23 (UTC) 使用 *
