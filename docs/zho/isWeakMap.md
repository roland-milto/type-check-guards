...
确定给定的 `value` 是否为 `WeakMap` 实例。

### 使用场景

当你接收一个 `unknown` 值（例如来自公共 API、插件系统或动态配置）并且需要在使用 `WeakMap` 特有行为之前验证它是 `WeakMap`
时，使用 `isWeakMap`。

> **给 TypeScript 用户的提示：**
>
> `isWeakMap` 执行 `instanceof WeakMap` 检查；它是一个运行时守卫，仅对实际的 `WeakMap` 实例返回 `true`。
...
<small>该文件于 2026年1月30日 13:27:45 (UTC) 使用 *
