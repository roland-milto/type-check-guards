...
`areIntegers` 用于判断给定数组中的所有元素是否都是整数；若是则返回 `true`，否则返回 `false`。
...
> **给 TypeScript 用户的提示：**
>
> 在将 `unknown[]` 输入当作仅包含整数的 `number[]` 使用之前，将 `areIntegers` 用作运行时守卫。如果它返回 `false`
> ，则该输入要么不是一个已填充的数组，要么至少包含一个非整数值。

### 优势

- 仅当每个元素都是整数时才返回 `true`；否则返回 `false`。
...
<small>该文件于 2026年1月31日 01:08:17 (UTC) 使用 *
