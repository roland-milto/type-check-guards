...
`areOctals` 用于判断提供的值是否为由有效八进制字符串组成的非空数组。

### 使用场景

当你需要验证用户输入、配置值或 API 载荷必须包含八进制字面量（例如文件权限模式 `0o755`），并且希望拒绝空数组或任何无效条目（返回
`false`）时，使用 `areOctals`。

> **给 TypeScript 用户的提示：**
>
> 使用 `areOctals` 来确保你拥有一个非空的 `unknown[]`，且其中每个条目都是有效的八进制字符串，然后再进行转换（例如通过
`Number(...)` 或自定义解析）。
...
- 在解析或转换八进制字符串之前作为守卫很有用，可避免运行时错误和不一致的输入处理。
...
<small>该文件于 2026年1月30日 14:59:14 (UTC) 使用 *
