...
`areMaps` 用于判断给定数组是否非空，并且其所有元素都是 `Map` 实例。

### 使用场景

在将未知输入（例如来自 JSON 解析、外部 API 或动态来源）当作非空的 `Map` 对象列表使用之前进行验证。
...
- 确保每个元素都是 `Map` 实例，只有在整个数组都通过检查时才返回 `true`。
- 按设计拒绝空数组，防止将“无数据”意外当作有效输入接受。
- 适合作为在集合上执行 `Map` 特定操作（例如 `.get()`、`.set()`、迭代）之前的守卫。
...
if (areMaps(items)) {
  ...
  // 对于：空数组，或包含任何非 Map 值的数组，结果为 false
}
...
<small>该文件于 2026年1月31日 16:15:23 (UTC) 使用 *
