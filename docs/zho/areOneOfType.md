...
`areOneOfType` 用于检查非空数组中的所有元素是否属于指定的运行时类型之一。
...
> **给 TypeScript 用户的提示：**
>
> 此函数返回布尔值，并且不会在编译时收窄数组元素类型；请将其用作进一步处理前的运行时校验步骤。

### 优势

- 确保数组中的每个元素都至少匹配一种允许的运行时类型，只有当整个数组都通过时才返回 `true`。
- 尽早拒绝无效输入：当 `array` 或 `types` 为空或不是已填充的数组时返回 `false`。
...
<small>该文件于 2026年1月31日 23:39:23 (UTC) 使用 *
