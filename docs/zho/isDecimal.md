...
`isDecimal` 用于检查某个值是否为十进制字符串表示：不包含前后空白，并且十进制格式有效。

### 使用场景

在解析或存储之前，验证表单字段、API 载荷或配置值：它们必须以十进制字符串（可选带符号）提供，且周围不能有任何空白字符。
...
<small>该文件于 2026年1月31日 15:55:29 (UTC) 使用 *
