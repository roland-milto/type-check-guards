...
- 支持可选的正负号，并且对前缀与数字大小写不敏感，可预测地返回 `true`/`false`。
...
isHexadecimal("1A2B"); // false（缺少前缀）
isHexadecimal(" 0x1A2B"); // false（前导空白）
isHexadecimal(0x1a2b); // false（不是字符串）
```
...
<small>该文件于 2026年1月31日 23:02:11 (UTC) 使用 *
