...
`areBigInts` 用于判断某个值是否为非空数组，且仅包含 `bigint` 值。

### 使用场景

验证未知输入（例如解析得到的类 JSON 数据、API 负载，或类型为 `unknown` 的函数参数），以确保在处理之前它是一个由 `bigint`
值组成的非空数组；仅当所有元素都是 `bigint` 时返回 `true`，否则返回 `false`。

> **给 TypeScript 用户的提示：**
>
> 在对未知输入执行仅适用于 `bigint` 的操作（例如算术运算、比较）之前，将 `areBigInts` 用作运行时守卫。
...
<small>该文件于 2026年1月31日 23:27:59 (UTC) 使用 *
