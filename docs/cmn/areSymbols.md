...
检查输入是否为一个已填充数组且其元素全部为 symbol，并返回 `true` 或 `false`。

### 使用场景

在使用需要 `symbol[]` 的 API 之前，验证某个配置字段（例如：用 symbol 表示的一组唯一键列表）是一个非空数组且只包含 symbol。

> **给 TypeScript 用户的提示：**
>
> 在将未知输入当作 `symbol[]` 使用之前，用 `areSymbols` 进行验证；对于非数组和空数组它会返回 `false`。

### 优势

- 仅当输入是非空数组且每个元素都是 symbol 时才返回 `true`。
- 通过内部的已填充数组检查拒绝非数组和空数组，从而防止误判。
- 作为运行时类型守卫很有用，可在进一步处理前验证仅包含 symbol 的列表。
...
<small>该文件于 30 January 2026 at 14:21:06 (UTC) 使用 *
