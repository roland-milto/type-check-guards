...
在处理每个 set 之前，验证某个值（例如来自用户输入、JSON 解析或外部 API）是否为由 `Set` 对象组成的非空数组。
...
- 仅当输入是非空数组且每个元素都是 `Set` 实例时才返回 `true`。
- 当数组没有任何元素时返回 `false`，从而避免对空数组产生误判为真。
- 在对每个元素执行 `Set` 特有操作之前，作为运行时守卫很有用。
...
<small>该文件于 30 January 2026 at 23:12:51 (UTC) 使用 *
