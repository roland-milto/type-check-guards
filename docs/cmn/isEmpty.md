...
判断给定值是否为空：对 `null`、`undefined`、空字符串/仅空白字符串、空数组、空 `Map`/`Set`，或没有自有可枚举属性的对象返回
`true`。
...
> **给 TypeScript 用户的提示：**
>
> `isEmpty` 是一个返回布尔值的工具函数（不是 TypeScript 类型谓词），因此它本身不会收窄类型；请将其用于校验/分支逻辑，而不是用于编译期类型收窄。

### 优势

- 在空值检查中将 `null` 和 `undefined` 视为 `true`（为空）。
- 在检查长度前先进行裁剪（trim），将仅包含空白字符的字符串视为空。
- 支持常见容器类型（数组、`Map`、`Set`）以及没有自有可枚举属性的普通对象。
- 通过使用 `hasOwnProperty` 检查来避免统计继承属性。
...
<small>该文件于 6 February 2026 at 16:17:54 (UTC) 使用 *
