...
- 仅当每个元素都是有效的 `Date` 实例时才返回 `true`（不包含像 `new Date('invalid')` 这样的无效日期）。
- 通过返回 `false` 来拒绝空输入，确保你只接受有意义的、非空的日期列表。
- 提供一种简单的布尔值守卫式检查，易于与其他校验组合使用。
...
<small>该文件于 30 January 2026 at 14:31:12 (UTC) 使用 *
