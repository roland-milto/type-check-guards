...
在迭代、访问第一个元素或应用需要至少一个条目的逻辑之前，使用 `isFilledArray` 来验证传入数据（例如 API 负载、表单值、配置）。

> **给 TypeScript 用户的提示：**
>
> `isFilledArray` 是一个运行时守卫，返回布尔值；除了确认数组非空之外，它不会进一步收窄元素类型。
...
<small>该文件于 6 February 2026 at 11:46:41 (UTC) 使用 *
