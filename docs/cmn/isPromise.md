...
在将未知输入当作 `Promise` 处理之前，使用 `isPromise` 进行校验，例如在处理来自插件、动态导入或类型较宽松的 API 的返回值时。
...
- 有助于保护那些需要真实 `Promise` 实例的代码路径，可预测地返回 `true` 或 `false`。
...
<small>该文件于 30 January 2026 at 23:52:28 (UTC) 使用 *
