...
> **给 TypeScript 用户的提示：**
>
> 在调用流方法之前，使用 `isStream` 来收窄 `unknown`；当这些全局对象存在时，它会识别 Node.js 类流对象（通过 `pipe`/`on`）以及
> Web Streams（`ReadableStream`/`WritableStream`）。

### 优势

- 通过检查 `pipe` 和 `on` 函数来安全地检测常见的 Node.js 类流对象。
- 在可用时，也会通过识别 `ReadableStream` 和 `WritableStream` 来支持 Web Streams。
...
<small>该文件于 30 January 2026 at 23:39:14 (UTC) 使用 *
