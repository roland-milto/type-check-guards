...
> **给 TypeScript 用户的提示：**
>
> 当你需要严格验证一个数组非空且只包含布尔值 `false` 时，使用 `areFalse`。

### 优势

- 确保每个元素都严格为 `false`（不进行 truthy/falsey 的强制转换）。
- 通过 `isFilledArray` 要求数组为已填充数组，从而对非数组或空数组返回 `false`。
- 为提高效率，在遇到第一个非 `false` 的元素时会提前停止。
...
<small>该文件于 31 January 2026 at 16:16:35 (UTC) 使用 *
