...
在将用户提供或动态值（例如配置、API 负载、插件输入）当作正则表达式处理之前先进行验证。

> **给 TypeScript 用户的提示：**
>
> 在使用 RegExp 特有的属性或方法之前，使用 `isRegEx` 来缩小 `unknown`（或联合类型）值的范围；它仅在值是 `RegExp` 的实例时才返回
`true`。
...
<small>该文件于 30 January 2026 at 23:28:56 (UTC) 使用 *
