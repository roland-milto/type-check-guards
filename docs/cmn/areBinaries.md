...
> **给 TypeScript 用户的提示：**
>
> 在将二进制字符串转换为 numbers/BigInts 之前，使用 `areBinaries` 来验证未知输入；它可确保数组非空且每个元素都是有效的二进制字符串。

### 优势

- 验证某个值是否为非空数组，且其中每个元素都是有效的二进制字符串。
- 返回简单的布尔结果（`true`/`false`），适用于类型守卫、提前返回和输入验证。
...
<small>该文件于 31 January 2026 at 23:14:08 (UTC) 使用 *
