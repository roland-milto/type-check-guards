...
`isFloat` 用于判断给定的 `value` 是否为有限的浮点数（不是整数的 `number`）。

### 使用场景

验证用户提供的数值输入在需要小数值的场景（例如价格、测量值、费率），并拒绝整数、`NaN` 和无穷大。
...
- 使用内置的数值守卫（`Number.isInteger`、`Number.isFinite`），行为可预测。
...
<small>该文件于 30 January 2026 at 16:07:09 (UTC) 使用 *
