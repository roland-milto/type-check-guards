...
验证未知数据（例如解析后的 JSON 或 API 响应），确保某个值在迭代、索引或访问 `.length` 之前是数组。
...
- 返回简单的布尔结果（`true`/`false`），适用于守卫和分支逻辑。
- 由于参数类型为 `unknown`，因此可用于任何输入类型。
...
<small>该文件于 6 February 2026 at 11:30:00 (UTC) 使用 *
