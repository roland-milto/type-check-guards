...
当你收到一个 `unknown[]`（例如来自 JSON、查询参数或外部 API），并且在运行诸如求平均、插值或统计计算等数值逻辑之前，需要确保它是一个已填充且每一项都是浮点数的数组时，使用
`areFloats`。

> **给 TypeScript 用户的提示：**
>
> 在将 `unknown[]` 当作仅包含浮点数的 `number[]` 处理之前，使用 `areFloats` 进行守卫；对于空数组以及任何非浮点元素，它都会返回
`false`。
...
<small>该文件于 30 January 2026 at 15:56:48 (UTC) 使用 *
