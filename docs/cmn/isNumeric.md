...
`isNumeric` 通过将给定 `value` 的解析类型与 `NUMERIC_TYPES` 进行对比，来判断其是否被视为数值。
...
- 使用 `getTypeOf` 与 `NUMERIC_TYPES` 来集中数值检测逻辑，并在整个代码库中保持检查一致。
...
<small>该文件于 6 February 2026 at 15:52:05 (UTC) 使用 *
