...
> آرایه‌های خالی و غیرآرایه‌ها مقدار `false` برمی‌گرداند.

### مزایا

- اطمینان می‌دهد که هر عنصر در آرایهٔ ورودی یک `WeakSet` است.
- برای آرایه‌های خالی مقدار `false` برمی‌گرداند و از نتایج تصادفیِ «همه معتبرند» در صورت نبود داده جلوگیری می‌کند.
- با برگرداندن `false` وقتی ورودی یک آرایهٔ پُر نباشد (از جمله `null`)، به‌صورت ایمن شکست می‌خورد.
...
<small>این فایل در ۱۰ بهمن ۱۴۰۴ ساعت ۱۴:۰۸:۲۷ (UTC) با استفاده از *
