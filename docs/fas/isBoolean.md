...
اعتبارسنجی داده‌های خارجی یا بدون نوع (مثلاً متغیرهای محیطی، payloadهای JSON، پارامترهای query) برای اطمینان از اینکه
مقدار از نوع `boolean` است، پیش از استفاده از آن در منطق شرطی.
...
- یک نتیجه `boolean` قابل پیش‌بینی (`true`/`false`) برمی‌گرداند.
...
<small>این فایل در ۱۰ بهمن ۱۴۰۴ ساعت ۱۴:۳۶:۱۵ (UTC) با استفاده از *
