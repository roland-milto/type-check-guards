...
مشخص می‌کند آیا یک مقدار، یک رشتهٔ لیترال هشت‌هشتی (octal) معتبر است یا نه (مثلاً `0o755`).

### مورد استفاده

اعتبارسنجی ورودی کاربر یا مقادیر پیکربندی که باید به‌صورت یک رشتهٔ لیترال هشت‌هشتی بیان شوند (برای مثال، حالت‌های مجوز
فایل مانند `0o644`) پیش از تجزیه (parse) یا تبدیل آن‌ها.

> **نکته برای کاربران TypeScript:**
>
> `isOctal` یک نگهبان نوع است (`value is string`). پس از نتیجهٔ `true`، TypeScript متغیرِ بررسی‌شده را به `string` محدود
> می‌کند.

### مزایا

- یک نگهبان نوع (type guard) سخت‌گیرانه فراهم می‌کند: فقط زمانی `true` برمی‌گرداند که ورودی یک رشته باشد که با قالب یک
...
<small>این فایل در ۱۰ بهمن ۱۴۰۴ ساعت ۱۴:۴۵:۰۰ (UTC) با استفاده از *
