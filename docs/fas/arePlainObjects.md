...
اعتبارسنجی داده‌های خارجی یا بدون نوع (مثلاً JSON پارس‌شده، payloadهای API، ارسال‌های فرم) برای اطمینان از این‌که یک
آرایهٔ غیرخالی دریافت کرده‌اید که هر ورودی آن یک شیء ساده است، پیش از آن‌که روی آن پیمایش کنید و ویژگی‌ها را بخوانید.

> **نکته برای کاربران TypeScript:**
>
> از `arePlainObjects` برای اعتبارسنجی ورودی ناشناخته استفاده کنید، پیش از آن‌که در TypeScript با آن مانند
`Record<string, unknown>[]` (یا یک شکل شیء سخت‌گیرانه‌تر) رفتار کنید.

### مزایا

- اطمینان می‌دهد هر عنصر در آرایهٔ ورودی یک شیء ساده (plain object) است و فقط زمانی `true` برمی‌گرداند که همهٔ آیتم‌ها
  مطابقت داشته باشند.
...
<small>این فایل در ۱۰ بهمن ۱۴۰۴ ساعت ۱۶:۲۵:۰۴ (UTC) با استفاده از *
