...
`isArray` بررسی می‌کند که آیا یک مقدارِ داده‌شده آرایه است یا نه و اگر آرایه باشد `true` و در غیر این صورت `false`
برمی‌گرداند.

### مورد استفاده

اعتبارسنجی داده‌های ناشناخته (مثلاً JSON تجزیه‌شده یا پاسخ‌های API) برای اطمینان از اینکه یک مقدار آرایه است، پیش از
...
if (isArray(input)) {
  //‎‎ ورودی در زمان اجرا یک آرایه است
...
<small>این فایل در ۱۷ بهمن ۱۴۰۴ ساعت ۱۱:۳۰:۱۶ (UTC) با استفاده از *
