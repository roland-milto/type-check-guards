...
`areWeakMaps` بررسی می‌کند آیا یک مقدار، آرایه‌ای غیرخالی است که هر عنصر آن یک `WeakMap` باشد؛ فقط در همان حالت `true` و
...
یک آرایهٔ غیرخالی از نمونه‌های `WeakMap` است، پیش از آن‌که روی آن پیمایش کنید و متدهای `WeakMap` را فراخوانی کنید؛ وقتی
هر عنصری `WeakMap` نباشد یا آرایه خالی باشد `false` برمی‌گرداند.

> **نکته برای کاربران TypeScript:**
>
> از `areWeakMaps` برای اعتبارسنجی ورودی ناشناخته پیش از برخورد با آن به‌عنوان یک `WeakMap[]` غیرخالی استفاده کنید؛ برای
> آرایه‌های خالی `false` برمی‌گرداند.

### مزایا

- اطمینان می‌دهد هر عنصر در آرایهٔ ارائه‌شده یک نمونهٔ `WeakMap` است.
...
<small>این فایل در ۱۰ بهمن ۱۴۰۴ ساعت ۱۳:۳۷:۰۲ (UTC) با استفاده از *
