...
Valider innkommende data (f.eks. fra query params, miljøvariabler eller eksterne API-er) der du forventer en array med
JSON-kodede strenger og vil avvise tomme arrayer eller eventuelle ikke-JSON-oppføringer.

> **Merknad for TypeScript-brukere:**
>
> Bruk `areJson` når du må validere at en `unknown[]` kun inneholder JSON-strenger før du parser dem (f.eks. med
`JSON.parse`).
...
const mixed = areJson(["{\"a\":1}", 123 as unknown]); // usant
const empty = areJson([]); // usant
...
<small>Filen ble generert 30 January 2026 at 16:16:57 (UTC) ved bruk av *
