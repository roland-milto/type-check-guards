...
Valider at en verdi (f.eks. fra brukerinput, JSON-parsing eller eksterne API-er) er en ikke-tom matrise av `Set`
...
if (areSets(a)) {
  // a er en matrise av Set-instanser ved kjøring
  for (const s of a) {
    console.log(s.size);
  }
}
...
<small>Filen ble generert 30 January 2026 at 23:14:44 (UTC) ved bruk av *
