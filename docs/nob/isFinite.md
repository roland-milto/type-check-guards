...
- Bruker den innebygde `Number.isFinite` for en pålitelig sjekk av om noe er endelig.
- Returnerer `true` kun for endelige tall; returnerer `false` for `NaN`, `Infinity` og inndata som ikke er tall.
- Enkel predikatfunksjon uten bivirkninger, egnet for validering og beskyttende logikk.
