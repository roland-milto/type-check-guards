...
returnerer `true` hvis det stemmer og `false` ellers.

### Brukstilfelle

Valider innkommende chunk-arrayer (f.eks. fra strømmer, filopplastinger eller nettverkspakker) for å sikre at alle deler
er `Buffer`-instanser før du setter dem sammen, dekoder eller sender dem videre til kryptografiske eller
binærbehandlende funksjoner.
...
- Avviser ugyldige inndata tidlig ved å kreve et ikke-tomt, utfylt array; returnerer `false` for tomme arrayer eller
  ikke-arrayer.
- Nyttig som en guard før du utfører operasjoner som kun gjelder buffere (f.eks. sammenkjedning, hashing, binære
  protokoller).
