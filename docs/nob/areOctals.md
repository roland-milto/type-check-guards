...
`areOctals` avgjør om den angitte verdien er en ikke-tom matrise med gyldige oktale strenger.

### Brukstilfelle

Bruk `areOctals` når du validerer brukerinput, konfigurasjonsverdier eller API-payloads som må inneholde oktale
...
- Nyttig som en guard før parsing eller konvertering av oktale strenger for å unngå kjøretidsfeil og inkonsekvent
...
if (Array.isArray(value) && areOctals(value)) {
  // value er en ikke-tom matrise med oktale strenger
