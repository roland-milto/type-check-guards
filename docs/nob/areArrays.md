...
Bruk `areArrays` til å validere tabell- eller matrise-lignende inndata (f.eks. CSV-rader, rutenettdata eller grupperte
...
> over eller indekserer i nøstede matriser.

### Fordeler

- Validerer at inndata er en ikke-tom todimensjonal matrise der hvert element er en matrise.
- Returnerer et enkelt boolsk resultat (`true`/`false`) som passer for guards og tidlige avbrudd.
- Hjelper med å forhindre kjøretidsfeil når senere kode forutsetter nøstede matriseoperasjoner (f.eks. mapping av
  rader).
...
if (areArrays(value as unknown[][])) {
  // value er en 2D-matrise med matriser som elementer
...
<small>Filen ble generert 6 February 2026 at 13:40:48 (UTC) ved bruk av *
