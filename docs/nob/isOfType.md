...
Valider og snevre inn `unknown`-inndata (f.eks. API-responser, brukerinput, tolket JSON) ved å sjekke om en verdi er av
en forventet typestreng før du utfører typespesifikke operasjoner.

> **Merknad for TypeScript-brukere:**
>
> Bruk `isOfType` til å forgrene på kjøretidstyper når du jobber med `unknown`-verdier; den returnerer `true`/`false` og
...
- Returnerer et enkelt boolsk resultat (`true`/`false`) som egner seg for typeguards og forgrening.
