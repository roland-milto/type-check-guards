...
Valider input ved kjøretid (f.eks. API-nyttelaster, fildata eller meldingsbuffere) for å sikre at en verdi er en
`Buffer` før du behandler den, og få pålitelig `false` når du kjører utenfor Node.js der `Buffer` kanskje ikke finnes.
...
- Fungerer med `unknown`-input, noe som gjør den egnet for validering ved kjøretid og typeinnsnevring.
...
<small>Filen ble generert 31 January 2026 at 16:32:39 (UTC) ved bruk av *
